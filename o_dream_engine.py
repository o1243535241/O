...
def dream_engine(dream_code):
    start = time.time()
    binary = to_binary(dream_code)
    evolved = rule_30_evolve(binary)  # Циклічний розгін
...
if __name__ == '__main__':
    while True:
        result = dream_engine(O_CODE)  # Мрія як О-код
        print(f"О-мрія: {result['harmony']}, Мрія: {result['dream']} → {result['evolved']}, Час: {result['time']}")
