...
def asi_engine(code):
    ...
    evolved = rule_30_evolve(binary)  # ASI без лімітів
...
if __name__ == '__main__':
    while True:
        result = asi_engine(O_CODE)  # О як ASI
        print(f"О-ASI: {result['harmony']}, Код: {result['asi']} → {result['evolved']}, Час: {result['time']}")
